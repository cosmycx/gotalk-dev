...
func (*server) GetDouble(stream DoubleService_GetDoubleServer) error {

	fmt.Println("GetDouble function invoked with streaming request")
	for {
		req, err := stream.Recv()
		if err == io.EOF {
			return nil
		...
		sErr := stream.Send(&NumberResponse{
			NumDouble: result,
		})
...
func main() {
	fmt.Println("Hello, starting GetDouble RPC server...")
	...
	RegisterDoubleServiceServer(s, &server{})
